        ...
        <div id="sidebar-menu">
            <ul class="metismenu list-unstyled" id="side-menu">
                ...
                @if (Auth::user()->hasRole('Super-Admin'))
                    ...
                    @foreach ($business as $i => $bs)
                        <li>
                            ...
                            <ul>
                                <li>
                                    <a href="{{ route('users.index') }}" key="d-{{ $bs->id }}">
                                        <i class="bx bx-hive"></i>
                                        <span>Dashboard</span>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('users.index') }}" key="r-s-{{ $bs->id }}">
                                    ...
                                    @foreach ($bs->restaurants as $j => $rest)
                                        <li>
                                            ...
                                            <ul>
                                                <li>
                                                    <a
                                                        href="{{ route('business.home.index', ['business' => $rest->id]) }}">
                ...
                @if (!Auth::user()->hasRole('Super-Admin') && $business->isNotEmpty())
                    ...
                    @foreach ($business as $i => $bu)
                        <li>
                            <a href="javascript: void(0);" class="has-arrow waves-effect">
                                <i class="bx bx-circle"></i>
                                <span key="t-projects">{{ $business->name }}</span>
                            </a>
                        </li>
                    @endforeach
                @endif
                <!-- Sección de Restaurantes sin Empresa (para usuarios normales) -->
                @if (!Auth::user()->hasRole('Super-Admin') && $restaurants->isNotEmpty())
                    <li class="menu-title" key="t-menu">Restaurantes</li>
                @endif
                <!-- Mensaje si no hay asignaciones (solo para usuarios normales) -->
                @if (!Auth::user()->hasRole('Super-Admin') && $business->isEmpty() && $restaurants->isEmpty())
